...
# 门禁卡 ruff F + S108 + I + TC 与 mypy strict（均已全仓清零）；全量 ruff 尚有 197 条既有欠账（E501 为主），别被吓到
...
uv run --extra dev --extra litellm --extra telemetry-otel mypy src/  # ← 门禁也跑这条
```
...
`ruff check --select F,S108,I,TC src tests examples scripts` + `mypy src/`。本地跑这三条就等于预跑了门禁。**门禁不跑真实 LLM**——真实回归仍是
