...
| [post_turn_review/](post_turn_review/) | **post_turn 钩子**:turn 收尾自我审计 / 记忆固化 —— 每轮真终态后在下一轮前**确定性**固化结论（顺序保证;钩子 vs 异步事件订阅） |
...
| [compression_showcase/todo_demo.py](compression_showcase/todo_demo.py) | **todo builtin**（LLM 经 `todo_write` 整表维护清单,双注入装配自动穿越压缩） | 否（Mock） |
