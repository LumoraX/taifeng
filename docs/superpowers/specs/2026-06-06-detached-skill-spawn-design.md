...
- **HITL 路由**:专家在自己 child thread 落挂起 record,发 `spawn_suspended`(= 该 child thread 的 turn_suspended,data 带 `handle_id` + `thread_id`)。业务 `Resume(thread_id=child, resolutions=…)` → engine 在 `Resume` 派发分支先用 `_match_suspended_spawn(thread_id)` 命中**挂起态**句柄 → 路由到**专用 `_resume_spawn`**(`asyncio.create_task`,与 `_drive_spawn` 一样在该 child thread 自己的线上独立续跑)→ 续到终态 → `spawn_completed`/`spawn_suspended`(再挂起则句柄重标 suspended,可再 resume,支持多轮错峰 HITL)。
  - **为何不直接复用 `_handle_child_resume`**:后者的前提是「父 turn 此刻正挂在 `CHILD_SKILL` gap 上」,会自根沿 `CHILD_SKILL` pending 串链下探、并逐层把子结果回填父 `call_skill` 的 `function_call_output`。detached spawn 的父 turn **早已结束**、根本不存在这条续跑链;强行复用要给它塞「无父链」分支,反而把已经很复杂的续跑链逻辑搅浑。故写一个聚焦的 `_resume_spawn`:① `load_thread` 子 thread → `_find_active_suspension_in` 找活跃挂起;② 复用 `SuspensionResolver`(强制全量 resume,杜绝部分 resume 违例)+ `_apply_plan_on_thread` 补 gap 落 marker;③ 复用 `_build_child_runner`(call_stack 仍空 → 子 turn 仍是**独立根 turn**,新增 `history=` 参数从 store 历史起跑)续到终态;④ 复用 `_finalize_spawn` 收尾,与首发路径完全一致。`_apply_plan_on_thread` / `_find_active_suspension_in` / `SuspensionResolver` / `_build_child_runner` / `_finalize_spawn` 全部沿用,新增代码只是把它们按「无父链的独立 child」编排到一起。
