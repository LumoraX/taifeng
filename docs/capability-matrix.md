...
| **Lineage peer messaging + any-of-N wait** | Point-to-point delivery inside a thread lineage (queue-only / trigger-turn, wakes idle terminal children); three wait tiers — `wait_peer` (one), `wait_any` (any, collecting every handle already terminal at wake), `await_skills` (all, then aggregate) | `send_message` / `wait_peer` / `wait_any` tools, `SendToPeer` op | [multi_expert_consult/](../examples/multi_expert_consult/) | [peer-mailbox-messaging.md](architecture/capabilities/peer-mailbox-messaging.md) ✅ | [`peer_messaging`](real-llm-ledger.md) · [`wait_any`](real-llm-ledger.md) |
...
| **Tool whitelist consistency** | Visible tools have one source of truth; scripts are folded into `run_script`, dispatch rejects `not_offered` calls, and `strict_tool_names: true` narrows an entry to its declared tools by suppressing the auto-injected `read_skill` / `call_skill` | `SkillDefinition.visible_tool_names()` / `dispatch_batch(visible_tools=)` | [travel_planner/](../examples/travel_planner/) | [tool-whitelist.md](architecture/capabilities/tool-whitelist.md) ✅ | [`travel_planner`](real-llm-ledger.md) |
...
| **Cache-friendly contract** | Every compaction reports `cache_invalidated` and `anchor_preserved_until`; the anchor is the inclusive index of the last provider-cached item, advances after every successful sample, and every strategy mutates only `anchor+1` onward; overflow recovery is two-stage (tail-only first, head allowed with an expected `compaction_overflow` break) | `CompressionResult` / `cache_break_detected` event / `ApiRequest.cache_breakpoints` | [observability/](../examples/observability/) | [cache-anchor.md](architecture/capabilities/cache-anchor.md) ✅ | 🧪 sim-covered (`tests/loop/test_cache_anchor_truth.py`); [capability_matrix.py](../examples/real_llm/capability_matrix.py) compression scenario |
...
| **Independent Codex Responses provider** | Explicit `provider=codex`; top-level instructions, typed input list, image/tool input, done-item terminal facts, isolated encrypted reasoning state, safe request audit, cancellation, legacy JSONL cold resume, and counted-and-skipped tolerance for non-protocol SSE frames (relay keepalives) while protocol violations stay fail-closed; no Chat fallback or hidden OpenAI switching | `CodexResponsesClient` / `ImageInputPolicy` | [Codex real matrix](../examples/real_llm/test_codex_image_matrix.py) · [SSE noise](../tests/llm/test_codex_sse_noise.py) | [llm-codex-provider.md](architecture/capabilities/llm-codex-provider.md) ✅ | `codex_*` in [ledger](real-llm-ledger.md); `NOT_EXECUTED` without an authorized proxy key/base URL |
| **Tool image attachments** | Tools may return images to the model: `ToolResult.attachments`, admission before durable append, separate `tool_output_modalities` capability tier, native `input_image` inside `function_call_output` on Responses/Codex, in-band placeholder degradation elsewhere, routing-time `requires.modalities` gate, budget accounting, and compression attachment-awareness | `ToolResult.attachments` / `ImageAttachmentV1.from_bytes` / `ImageInputPolicy` | [settlement e2e](../tests/loop/test_tool_image_settlement.py) | [tool-image-attachment.md](architecture/capabilities/tool-image-attachment.md) ✅ | `codex_tool_image_output` in [ledger](real-llm-ledger.md) —— 常驻场景，断言模型报出只存在于图内的几何信息（区分「wire 收下」与「模型看见」）；`NOT_EXECUTED` without an authorized proxy key/base URL |
...
| **Retry + error classification** | Exponential backoff, server hint delays, 12 error classes, and machine-readable recovery plans. Streaming retries go through `RetryingModelClient`, which retries only when the attempt produced no content and is deliberately mutually exclusive with strict audit's one-attempt contract; `retry_async` remains the non-streaming helper. Every backoff retry is emitted **before** the wait as a `provider_retry` event (`reason` = error kind, plus `attempt` / `max_attempts` / `delay_seconds` / `failure_class`), rendered by the console sink and counted by the OTel counter `taifeng.provider.retries` (ADR 0039) | `RetryingModelClient` / `retry_async` / `LLMError` / `RecoveryPlan` | [real_llm/](../examples/real_llm/) (on by default in `AgentEngine` / `EnginePool`, ADR 0041; `auto_retry=False` opts out, `retry_config=` tunes) | [llm-client.md](architecture/llm-client.md) | 🧪 sim-covered (`tests/llm/test_retrying_client.py`); exercised by every ledger run |
| **Provider circuit breaker** | Cross-turn upstream health: consecutive retry-exhausted failures trip the circuit, `open` fast-fails without touching the network, one half-open probe per cooldown, doubling and capped backoff on probe failure. Counts only final outcomes (cancellation and non-retryable errors are excluded), state lives on the decorator instance (one per endpoint, shared process-wide, never persisted), and all three transitions are emitted as dedicated events plus the OTel counter `taifeng.provider.circuit_transitions` (ADR 0042) | `CircuitBreakingModelClient` / `BreakerConfig` / `CircuitOpenError` | — | [provider-circuit-breaker.md](architecture/capabilities/provider-circuit-breaker.md) ✅ | 🧪 sim-covered (`tests/llm/test_circuit_breaker.py` 11 cases + `tests/loop/test_provider_circuit_observable.py` 4 cases); off by default, so no ledger scenario |
| **Untrusted finish_reason endpoints** | Gateways that collapse every unmapped upstream finish reason into `content_filter` are declared untrusted by the integrator, so a zero-output `content_filter` is classified as the retryable `UnreliableFinishError` instead of a terminal content block | `OpenAICompatClient(trust_finish_reason=False)` | — | [llm-client.md](architecture/llm-client.md) 🧪 | — |
...
- **Ordering is contractual**: `join_barrier_fired` is emitted *before* the aggregation turn starts, so it always precedes any event carrying `submission_id == then_thread_id`. A subscriber can therefore open the consultation track before its first token arrives; it never has to attribute aggregation output to an unknown track. See [detached-spawn.md](architecture/capabilities/detached-spawn.md) §顺序保证.
...
`read_skill` · `call_skill` · `file_read` · `file_write` · `shell_exec` · `apply_patch` · `http_request` · `run_in_background` · `wait_for_task` · `run_script` · `request_user_input` · `spawn_skill` · `await_skills` · `join_skill` · `kill_skill` · `send_message` · `wait_peer` · `wait_any` · `todo_write`
