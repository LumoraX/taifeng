...
| **分离式并发 spawn + join-barrier** | `spawn_skill` 立即返回句柄不阻塞、各 child 独立 HITL；`await_skills` 全终态自动聚合 | `spawn_skill` / `await_skills` / `join_skill` / `kill_skill` 工具 | [multi_expert_consult/](../examples/multi_expert_consult/) | [detached-spawn.md](architecture/capabilities/detached-spawn.md) ✅ |
