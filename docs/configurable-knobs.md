...
| `post_turn` | `AgentEngine._fire_post_turn_hook`（`_build_and_run_runner` 收尾）：状态回写后、下一 turn 前，仅 root turn 真终态（suspended/cancelled 跳过） | 审计型不可否决；deny/异常仅写日志；emit `post_turn_hook_fired`；R4 经 `ctx.extras["cancel"]` 传 token |
