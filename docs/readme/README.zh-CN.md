...
| 能力 | 模块 | spec |
|---|---|---|
| **统一 Skill 模型** —— atomic + composite + 静态/运行时环检测 | `skill/` | [skill-dispatch](../architecture/capabilities/skill-dispatch.md) |
| **Tool 系统** —— RwLock 并行 / 独占调度（`parallel_safe` 字段） | `tool/` | — |
| **内置工具集（10 个）** —— `read_skill` / `call_skill` / `file_read` / `file_write` / `shell_exec` / `apply_patch` / `run_in_background` / `wait_for_task` / `run_script` / **`http_request`** | `tool/builtins/` | [tool-builtins-extended](../architecture/capabilities/tool-builtins-extended.md) |
...
| **PermissionPolicy + HITL** —— Claude Code 风格 `Bash(...)` / `Network(...)` / `Skill(read_*)` 规则语法 | `permission/` | [permission-gate](../architecture/capabilities/permission-gate.md) |
| **MCP stdio client + server mode** —— 外部 MCP 自动注册 / 反向作为 server 暴露 skill | `mcp/` | [mcp-server](../architecture/capabilities/mcp-server.md) |
| **Cache-aware 压缩** —— Handoff（codex 范式）+ SlidingWindow + cache_stats | `context/strategies/` | — |
| **LLM 强类型输出** —— `ResponseFormatSpec` + `structured_output` 事件 + 3 provider 统一翻译 | `llm/` | [llm-structured-output](../architecture/capabilities/llm-structured-output.md) |
| **Subagent 隔离** —— inherit / auto_deny / auto_allow 三种 PermissionPolicy 包装模式 | `skill/dispatch.py` | — |
| **JSONL transcript + Resume** —— 追加写主存 + SQLite 旁路索引 + thread_id resume | `conversation/` | [jsonl-transcript](../architecture/capabilities/jsonl-transcript.md) |
| **Instructions 注入** —— CLAUDE.md / system_prompt / project_instructions 三层 resolver | `loop/prompt.py` | [instructions-injection](../architecture/capabilities/instructions-injection.md) |
| **Telemetry** —— ConsoleSink / JsonlSink / **OtelTelemetrySink**（OTLP exporter，业务侧按需开启） | `telemetry/` | [telemetry-otel](../architecture/capabilities/telemetry-otel.md) |
| **Script 执行（M4）** —— SKILL.md 内 `scripts:` 派发 Bash/Python，含启发式 deny list | `skill/scripts/` | [script-execution](../architecture/capabilities/script-execution.md) |
...
按 [ADR 0006「统一 Skill 模型」](../decisions/0006-unified-skill-model.md) —— 无独立 `agent/` 包；skill-to-skill 派发归 `skill/dispatch.py`，composite skill 替代 agent 概念。
...
- **622 测试** 全绿（pytest）
- **14k LOC** src，15 个能力契约（[`docs/architecture/capabilities/`](../architecture/capabilities/README.md)）
...
- **❓** Memory backends / Multi-agent handoff 显式 API —— 需先决定 R1 归属，详见 [hermes-gap-roadmap.md](../architecture/hermes-gap-roadmap.md)
...
四方范式对标的差距分析见 [docs/architecture/hermes-gap-roadmap.md](../architecture/hermes-gap-roadmap.md) 与 [kernel-gap-analysis.md](../architecture/kernel-gap-analysis.md)。
...
| [docs/architecture/overview.md](../architecture/overview.md) | 架构总览（模块 / 数据流 / 红线 / 里程碑） |
| [docs/configurable-knobs.md](../configurable-knobs.md) | 业务可配置参数全清单（含 §7 structured_output） |
| [docs/architecture/hermes-gap-roadmap.md](../architecture/hermes-gap-roadmap.md) | hermes capability gap 对齐路线图 |
| [docs/decisions/](../decisions/) | 10 个 ADR 决策记录 |
| [docs/architecture/capabilities/](../architecture/capabilities/README.md) | 15 个能力契约（数据结构 / 协议 / 事件 / 约束的权威定义） |
...
[ADR 0001 — 为什么叫泰逢](../decisions/0001-naming-taifeng.md)
