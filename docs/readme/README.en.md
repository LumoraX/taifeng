...
|---|---|---|
| **Unified Skill Model** — atomic + composite + static/runtime cycle detection | `skill/` | [skill-dispatch](../architecture/capabilities/skill-dispatch.md) |
| **Tool System** — RwLock parallel / exclusive scheduling (`parallel_safe` field) | `tool/` | — |
| **Builtin Tools (10)** — `read_skill` / `call_skill` / `file_read` / `file_write` / `shell_exec` / `apply_patch` / `run_in_background` / `wait_for_task` / `run_script` / **`http_request`** | `tool/builtins/` | [tool-builtins-extended](../architecture/capabilities/tool-builtins-extended.md) |
| **Hook Lifecycle** — PreToolUse / PostToolUse / PreCompact / PreTurn / PreSkillDispatch / PostSkillDispatch | `hooks/` | [hooks](../architecture/capabilities/hooks.md) |
| **PermissionPolicy + HITL** — Claude Code-style `Bash(...)` / `Network(...)` / `Skill(read_*)` rule syntax | `permission/` | [permission-gate](../architecture/capabilities/permission-gate.md) |
| **MCP stdio client + server mode** — auto-register external MCP tools / reverse-expose skills as a server | `mcp/` | [mcp-server](../architecture/capabilities/mcp-server.md) |
| **Cache-aware Compaction** — Handoff (codex paradigm) + SlidingWindow + cache_stats | `context/strategies/` | — |
| **LLM Structured Output** — `ResponseFormatSpec` + `structured_output` event + 3-provider unified translation | `llm/` | [llm-structured-output](../architecture/capabilities/llm-structured-output.md) |
| **Subagent Isolation** — three PermissionPolicy wrapping modes: inherit / auto_deny / auto_allow | `skill/dispatch.py` | — |
| **JSONL Transcript + Resume** — append-only primary store + SQLite secondary index + thread_id resume | `conversation/` | [jsonl-transcript](../architecture/capabilities/jsonl-transcript.md) |
| **Instructions Injection** — CLAUDE.md / system_prompt / project_instructions three-layer resolver | `loop/prompt.py` | [instructions-injection](../architecture/capabilities/instructions-injection.md) |
| **Telemetry** — ConsoleSink / JsonlSink / **OtelTelemetrySink** (OTLP exporter, opt-in) | `telemetry/` | [telemetry-otel](../architecture/capabilities/telemetry-otel.md) |
| **Script Execution (M4)** — dispatch Bash/Python via SKILL.md `scripts:` field, with heuristic deny list | `skill/scripts/` | [script-execution](../architecture/capabilities/script-execution.md) |
...
Per [ADR 0006 "Unified Skill Model"](../decisions/0006-unified-skill-model.md) — no separate `agent/` package; skill-to-skill dispatch lives in `skill/dispatch.py`; composite skills replace the agent concept.
...
Details in [docs/architecture/overview.md](../architecture/overview.md).
...
- **14k LOC** src, 15 capability contracts ([`docs/architecture/capabilities/`](../architecture/capabilities/README.md))
...
- **❓** Memory backends / Multi-agent handoff explicit API — needs R1 boundary decision first, see [hermes-gap-roadmap.md](../architecture/hermes-gap-roadmap.md)
...
Gap analysis against the four reference paradigms: [docs/architecture/hermes-gap-roadmap.md](../architecture/hermes-gap-roadmap.md) and [kernel-gap-analysis.md](../architecture/kernel-gap-analysis.md).
...
| [docs/architecture/overview.md](../architecture/overview.md) | Architecture overview (modules / data flow / red lines / milestones) |
| [docs/configurable-knobs.md](../configurable-knobs.md) | Full list of business-configurable knobs (includes §7 structured_output) |
| [docs/architecture/hermes-gap-roadmap.md](../architecture/hermes-gap-roadmap.md) | hermes capability gap roadmap |
| [docs/decisions/](../decisions/) | 10 ADR decision records |
| [docs/architecture/capabilities/](../architecture/capabilities/README.md) | 15 capability contracts (authoritative data structures / protocols / events / constraints) |
...
[ADR 0001 — Why "Taifeng"](../decisions/0001-naming-taifeng.md)
