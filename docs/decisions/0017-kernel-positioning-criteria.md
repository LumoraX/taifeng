...
类比操作系统:taifeng 提供内核机制(调度 / 上下文 / 取消 / 恢复 / 通信)与内核能力(模型认知原语),业务方在其上构建发行版。对标三家的正确用法是**在别人的实现里找我们内核缺的机制**,不是对齐功能清单。
...
辅助判据:规则 1/2 存疑时,以「宿主业务方或可预见的嵌入方是否真的会用到」二次裁决;无人拉动的协议扩展(如 E2 可插拔 slot)一律挂起等需求。
