...
`cache_breakpoints` 是关键 —— 告诉 provider「这些位置之前的内容应该被缓存」。Anthropic 支持显式 `cache_control`，OpenAI 暂不支持但保留字段以备未来。**坐标系契约**：`CacheBreakpoint.index` 是 **messages 下标**；`build_api_request` 负责把 `cache_anchor_index`（history 下标，含语义：`[0, anchor]` 为已缓存前缀，`-1` 无缓存；由采样成功推进 / 压缩回写，见 `capabilities/cache-anchor.md`）换算到 messages 坐标——`anchor >= 0` 时打点在最后一条来源 history 下标 `<= anchor` 的产出消息，前缀无产出消息或 `anchor == -1` 则不打点（history→messages 非 1:1：记账 item 跳过、同轮合并折叠）。`response_format` 非 None 时 provider 强制 LLM 返回符合 schema 的 JSON（详见 `capabilities/llm-structured-output.md`）。
...
同一 label 每个 attempt warn 一次。**协议内违规不受影响**：显式失败终态、身份漂移、配对缺失、
delta/done 不一致、completed 后的协议事件仍全部 fail closed——终态保证由 `finalize()` 的完成门与
done items 承担，噪声吞不掉输出事实。
...
**默认开启（ADR 0041）**：`AgentEngine` / `AgentEnginePool` / `EnginePool.create` 经 `with_default_retry` 自动包装，接入方零改动即得；`retry_config=` 注入策略、`auto_retry=False` 关闭。幂等三规则：已套过（`bounded_retry` 标记，台账录制等 `__getattr__` 透明包装可穿透）、strict audit 的 `AttemptObservableModelClient`、显式关闭，均原样返回——所以显式套仍可用，且与 audit 的互斥无需接入方记住。
...
- **`retryable_kinds` 是显式白名单**（只看 `LLMError.kind`，不读 `LLMError.retryable`——让「哪些 kind 会自动重发」在配置里一眼可见）；默认 `{rate_limit, transient_network, server_error, unreliable_finish}`，最后一类是接入方声明 `trust_finish_reason=False` 时网关错标的零产出终止（ADR 0041 并入）。
