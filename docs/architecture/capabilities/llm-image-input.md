...
- `CodexResponsesClient` 必须调用显式配置 API root 下的 `/responses`，使用 `input_image`，并规范性继承本
  契约的 admission、预算、持久化、压缩和脱敏规则；其顶层 instructions、done-item 终态和 provider-state
  隔离规则见 [Codex provider 契约](llm-codex-provider.md)。
...
- `OpenAICompatClient` 保持 text-only。图片 capability 只允许由 `OpenAIChatClient`、
  `OpenAIResponsesClient` 和 `CodexResponsesClient` 显式声明；任何客户端都不能根据模型名或域名自动打开。
...
- `enable_request_capture=True` 仍保留文字 prompt，但每个图片 part 必须替换为包含
  media type/size/SHA-256/detail 的 `content_redacted` descriptor；`encrypted_content` 必须替换为不含原值的
  provider-state marker。OTel 继续整条跳过 request capture。
- strict attempt observer 还必须按 [ADR 0027](../../decisions/0027-sensitive-llm-request-audit.md) 取得安全投影、
  RFC 6901 redaction manifest 与完整 canonical attempt digest；它们是 request-intent data minimization，
  不是 Timeline/export redaction。
...
图片默认禁用。业务必须同时选择具备 image capability 的专用协议客户端并显式注入策略；以下以 OpenAI
Responses 为例，Codex 使用 `CodexResponsesClient` 且 admission/estimator 注入相同：
