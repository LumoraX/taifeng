...
Taifeng 是一个**与业务解耦**的通用 LLM Agent 引擎——**LLM OS 的内核：用来开发开箱即用的产品，自身不是产品**。

立项裁决四规则（详见 [ADR 0017](../decisions/0017-kernel-positioning-criteria.md)）：① 内核机制缺口 → 做；② 模型认知回路需要的原语（自我 review / 任务清单工作记忆 / 状态穿越压缩）→ 做；③ 外部成熟服务能承担的（DB / 向量记忆 / 知识库）→ 内核只定协议、实现走外部；④ 仅"别家有"的产品功能 → 不做。
