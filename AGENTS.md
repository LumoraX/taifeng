...
- **CI 门禁**（`.github/workflows/ci.yml`，push main / 每个 PR 自动触发）：三个 job —— Python 3.12+3.13 全量 `pytest tests/`、`scripts/verify_examples.py` 的 examples 冒烟、`ruff check --select F,S108` 静态检查。本地跑这三条 = 预跑门禁。**只卡 ruff F 类 + S108**（未用导入 / 重名覆盖 / 未用变量 / 未定义名 / 生产代码硬编码 /tmp，零风格意见）；E501/TC/I 共 400+ 条既有欠账不入闸，要清得单独开切片。门禁只跑 sim，真实回归仍走下面的台账红线
