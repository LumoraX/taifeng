...
from taifeng.conversation.jsonl_atomic import (
    append_buffered,
    append_durable,
    describe_batch,
    exclusive_file_lock,
...
class JsonlMessageWriter:
    ...
    async def append(self, thread_id: str, items: list[ResponseItem]) -> None:
        ...
        async with lock, exclusive_file_lock(path.with_suffix(".lock")):
            await anyio.to_thread.run_sync(append_buffered, path, content)
    ...
    async def append_atomic_batch(
        ...
        *,
        ...
        descriptor = describe_batch(items, batch_id)
        path = self._thread_path(thread_id)
        lock = self._append_locks.setdefault(thread_id, anyio.Lock())
        async with lock, exclusive_file_lock(path.with_suffix(".lock")):
            ...
            await anyio.to_thread.run_sync(append_durable, path, content)
        return BatchAppendAck(
            batch_id=batch_id,
            digest=descriptor.digest,
            item_ids=descriptor.item_ids,
    ...
    async def _load_state(
        ...
        path = self._thread_path(thread_id)
        state = await anyio.to_thread.run_sync(read_state, path)
